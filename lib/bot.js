...
bot.on('message', async (message) => {
  ...
  switch (command) {
      ...
      if (charPayload) {
        ...
        switch (charCmd.toLowerCase()) {
          case 'roll':
            ...
            break;
          case 'report':
            logger.debug(`actionData: ${actionData.length}`);
            if (actionData.length > 0) {
              try {
                const charReporter = new CharacterReporter(charName);
                const charSkill = charReporter.reportOne(actionData);
                message.channel.send(`${charSkill.skillName}: ${charSkill.modifier}`);
              } catch (err) {
                message.channel.send(`I received an error: ${err}`)
                  .catch((error) => {
                    logger.error(`message error: ${error}`);
              ...
              try {
                ...
                message.channel.send(`Character Details:\nName: ${charAttr.name}\nRace: ${charAttr.race}\nClass: ${charAttr.class}\nLevel: ${charAttr.level}\nAttribute Modifiers:` +
                  `\n\tStr: ${charAttr.attributes.strength.value} (**${charAttr.attributes.strength.modifier}**) Dex: ${charAttr.attributes.dexterity.value} (**${charAttr.attributes.dexterity.modifier}**) Con: ${charAttr.attributes.constitution.value} (**${charAttr.attributes.constitution.modifier}**)` +
                  `\n\tInt: ${charAttr.attributes.intelligence.value} (**${charAttr.attributes.intelligence.modifier}**) Wis: ${charAttr.attributes.wisdom.value} (**${charAttr.attributes.wisdom.modifier}**) Cha: ${charAttr.attributes.charisma.value} (**${charAttr.attributes.charisma.modifier}**)`)
            ...
            break;
          default:
            message.channel.send(`I do not know command: ${charCmd}.`)
              .catch((err) => {
    ...
    case 'help':
      ...
      break;
    default:
      message.reply('I\'m sorry, but Human isn\'t my first language. For a list of commands I do know please say \"!help\".');
      break;
  }
});
